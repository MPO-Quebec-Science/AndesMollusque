---
title: "Contraints de missions Andes"
output: rmarkdown::html_document
nocite: '@*'
vignette: >
  %\VignetteIndexEntry{Contraints de missions Andes}
...
Ne sont pas dans la table `TRAIT_MOLLUSQUE` de la BD Access, mais est presente dans sur PSE.
...
La colonne `REM_ENGIN_MOLL` dans la table `ENGIN_MOLLUSQUE` existe sur PSE, mais n'est pas présente sur Access.
...
Il faudrait chercher (et valider) l'existance de type `0` et `3` dans ACCESS et PSE (respectivement).

#### CAPTURE_MOLLUSQUE
Certaine tables de reference n'ont pas les memes valeurs entre la BD de MS Access la BD Peche_Sentinelle (PSE). En cas de doute la BD PecheSentinelle devrait etre considéreé comme étant la bonne.

La colonne `REM_CATURE_MOLL` dans la table `ENGINCAPTURE_MOLLUSQUE_MOLLUSQUE` existe sur PSE, mais n'est pas présente sur Access.
