...
astuce: `ctrl+a` (tout selectioner) `ctrl+c` (copie) `ctrl+v` (coller)

### Pétoncle

Copier des tables suivantes (dans cette ordre) : 1. `PROJET_MOLLUSQUE`
(normalement une seul ligne) 2. `TRAIT_MOLLUSQUE` (tout) 3.
`ENGIN_MOLLUSQUE` (tout) 4. `CAPTURE_MOLLUSQUE` (tout) 5.
`FREQ_LONG_MOLLUSQUE` (tout)
