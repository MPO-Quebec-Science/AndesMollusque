...
get_projet_mollusque <- function(andes_db_connection) {
  ...
  proj <- add_hard_coded_value(
    proj,
    col_name = "DUREE_TRAIT_VISEE_P",
    value = 0.1
  )
  proj <- add_hard_coded_value(
    proj,
    col_name = "VIT_TOUAGE_VISEE_P",
    value = 0.1
  )
  proj <- add_hard_coded_value(
    proj,
    col_name = "RAPPORT_FUNE_VISEE_P",
    value = 1.0
  )
  proj <- add_hard_coded_value(
    proj,
    col_name = "DIST_CHALUTE_VISEE_P",
    value = 1.0
  )
  ...
  proj <- add_hard_coded_value(
    proj,
    col_name = "RAPPORT_FUNE_VISEE",
    value = 1.0
  )

  proj <- add_hard_coded_value(
    proj,
    col_name = "NOM_EQUIPE_NAVIRE",
    value = ""
  )
...
write_projet_mollusque <- function(proj, access_db_write_connection = NULL) {
  ...
  statement <- generate_sql_insert_statement(proj[1, -1], "PROJET_MOLLUSQUE")
  result <- DBI::dbExecute(access_db_write_connection, statement)

  if (result != 1) {
    logger::log_error(
      "Failed to write the projet_mollusque to the database."
    )
    stop("Failed to write the projet_mollusque to the database.")
  } else {
    logger::log_info(
      "Successfully wrote the projet_mollusque to the database."
    )
  }
  # DBI::dbClearResult(result)
}
